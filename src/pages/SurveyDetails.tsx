...
const SurveyDetails = () => {
  ...
  const handleRemoveFile = async (fileId: string) => {
    ...
    try {
      const response = await authenticatedFetch(`http://localhost:3000/api/surveys/files/${fileId}`, {
        method: 'DELETE'
      });
  ...
  const handleGenerateSuggestions = async () => {
    ...
    try {
      ...
      const res = await authenticatedFetch(`http://localhost:3000/api/surveys/${surveyId}/suggestions`, {
        method: 'POST',
        body: JSON.stringify({ 
                  ...
                  {isEditingCategory ? (
                    ...
                    <div className="flex items-center space-x-2 mt-1">
                      <Badge variant="secondary" className="capitalize bg-blue-500/20 text-blue-300">
                        {survey.category ? survey.category.replace('-', ' ') : 'Uncategorized'}
                      </Badge>
                      <Button 
                        size="sm" 
                        variant="ghost" 
